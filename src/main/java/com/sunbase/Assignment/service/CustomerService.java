...
//    public Object getCustomerList(String bearerToken) {
//        try {
//
//            HttpHeaders headers = new HttpHeaders();
//            headers.setContentType(MediaType.APPLICATION_JSON);
...
////            ResponseEntity.ok(Arrays.asList(Objects.requireNonNull(response.getBody())));
//
//             ResponseEntity<String> res = restTemplate.exchange(customerUrl + "?cmd=get_customer_list", HttpMethod.GET, entity, String.class);
//
//            System.out.println("this is a res " + res);
//
//            if (res.getStatusCode().is2xxSuccessful()) {
//                String responseBody = res.getBody();
//
//                // Use Jackson ObjectMapper to deserialize the JSON string into a list of Customer objects
...
//                    "}";
    ...
    public Customer getCustomerByUuid(String token, String uuid) throws JsonProcessingException {
        ResponseEntity<String> res = getCustomerList(token);
        ...
        if (res.getStatusCode().is2xxSuccessful()) {
            ...
            // Use Jackson ObjectMapper to deserialize the JSON string into a list of Customer objects
            ObjectMapper objectMapper = new ObjectMapper();
            customerList = objectMapper.readValue(responseBody, new TypeReference<List<Customer>>() {});
